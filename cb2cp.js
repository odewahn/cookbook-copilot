...
function slugify(s) {
  return s.replace(/\s/g, '-').replace(/[^\w-]+/g, '').replace(/\-\-+/g, '-').replace(/^-+/, '').replace(/-+$/, '');
}
...
data.getBlocks()[0].getBlocks().map((recipe, idx) => {
  if (recipe.title.length) {
      var functionKeyword = fLookup[language] || "function"
      var recipe_number = "recipe-" + ('000'+idx).slice(-3);
      var cookbookName = argv["f"].split(".")[0].toLowerCase().split("/").slice(-2).join("-");
      var title_slug = slugify(recipe.title.toLowerCase()) 
      var filename = cookbookName + "-" + recipe_number + "-" + title_slug + "." + functionKeyword[4];
